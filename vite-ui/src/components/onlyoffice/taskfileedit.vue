<template>
  <el-row style="width: 100%; height: 100%">
    <el-col :span="leftspan" v-show="letfshow" class="leftBox">
      <div class="tableBox">
        ...
        <el-table :data="fileTableList" :show-header="true">
          ...
          >
            <template #default="scope">
              ...
              <el-tag type="primary" v-if="scope.row.submitStatus == 2"
                >已提交</el-tag
              >
          ...
          >
            <template #default="scope">
              <el-button
                ...
                v-if="
                  scope.row.submitStatus == 0 || scope.row.submitStatus == 1
                "
                >重命名</el-button
              >
              <el-button
      ...
      <div class="tableBox">
        <el-row :gutter="10" class="mb8">
          <div class="title">基础想定：</div>
        </el-row>
        <el-table :data="fileStageTableList" :show-header="true">
          <el-table-column label="文件名称" align="center" prop="fileName" />
    ...
    <el-col :span="rightspan" style="display: flex; flex-direction: column">
      <el-row style="height: 40px">
        ...
        <el-button type="primary" @click="openTargetTeach">目标筹划</el-button> -->
...
<script setup>
import { useTaskSituationStore } from "@/store/taskSituationStore.js";
const situationStore = useTaskSituationStore();
...
import {
  delScenario,
  getScenarioBaseApi,
} from "@/api/textscenario/textscenario.js";
import {
  loadWordTemplate,
  startTrain,
  submitJob,
} from "@/api/missiondesign/trainmission.js";
import { getScenarioByTaskStageId } from "@/api/missiondesign/task.js"
...
import { SERVICE_URL } from "@/utils/targetRequest";
...
onMounted(() => {
  taskStageId.value = route.query.taskStageId;
...
function getTableList() {
  ...
  // 获取想定列表
  getScenarioByTaskStageId(taskStageId.value).then((response) => {
    if (response.code == 200) {
      fileStageTableList.value = response.data;
    }
  });
}
...
function getFile() {
  getUserProfile().then((response) => {
    ...
    startTrain(taskStageId.value).then((response) => {
      if (response.code == 200) {
        getTableList();
      }
    });
...
function loadtemplate() {
  if (worktemplate.value) {
    if (
      currentRow.value &&
      currentRow.value.id &&
      currentRow.value.submitStatus != 2
    ) {
      proxy.$modal
        .confirm("加载模板将使用模板覆盖当前文件，是否继续？")
        .then(function () {
          loadWordTemplate(currentRow.value.id, worktemplate.value).then(
            (response) => {
              if (response.code == 200) {
                option.value.key = generaterKey();
                ...
                axios.get("/config.json").then((response) => {
                  option.value.editUrl =
                    response.data.ONLYOFFICE_CALLBACKURL +
                    "?originurl=" +
                    response.msg;
                });
              }
...
function showfile(row) {
  ...
  axios.get("/config.json").then((response) => {
    option.value.editUrl =
      response.data.ONLYOFFICE_CALLBACKURL + "?originurl=" + row.file;
  });
}
...
const showScenarioFile = () => {
  getScenarioBaseApi(scenarioid.value).then((response) => {
    if (response.code == 200) {
      if (
        response.data &&
        response.data.textFile &&
        response.data.textFileName
      ) {
...
// 目标筹划
const openTargetTeach = () => {
  console.log(SERVICE_URL);
  if (SERVICE_URL === "/dev-api") {
    window.open(
      "http://192.168.1.30:3000/layout/fxdmbtxzy?jobId=" +
        scenarioid.value +
        "&isTeacher=1&token=" +
        getToken(),
      "targetTeach"
...
//提交作业
const submitwork = () => {
  submitJob(taskStageId.value).then((response) => {
